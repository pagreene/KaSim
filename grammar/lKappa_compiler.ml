...
let of_user_graph sigs g =
         ...
         let pack' =
           Tools.array_fold_lefti
             (fun id (dandling, free_id as pack) si ->
               match si.User_graph.site_type with
               ...
               | User_graph.Port p ->
                ...
                | [ s ] ->
                   ...
                   ra_ints.(id) <- LKappa.I_VAL_CHANGED (i,i)
                | _ :: _ :: _ ->
  ...
  let (mix_without_counters,raw) =
    Counters_compiler.remove_counter_rule sigs out [] in
  let () =
    if not(raw = []) then
      raise (ExceptionDefn.Internal_Error
               ("User_graph has no raw mixture",Locality.dummy)) in
  mix_without_counters
