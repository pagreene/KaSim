...
let empty_handler parameters error =
  ...
  in
  let error,dual =
    Ckappa_sig.Agent_type_site_state_nearly_Inf_Int_Int_Int_storage_Imperatif_Imperatif_Imperatif.create
      parameters error
      (0, (0, 0))
  in
  error,
        ...
        begin
          ...
          then
            ...
            let error, handler =
              error,
              {
                handler with Cckappa_sig.states_dic = states_dic
              }
            in
            error, (handler, l, k)
          else
...
let declare_dual parameter error handler ag site state ag' site' state'=
  ...
  let error, dual =
    Ckappa_sig.Agent_type_site_state_nearly_Inf_Int_Int_Int_storage_Imperatif_Imperatif_Imperatif.set
      parameter
      error
      (ag, (site, state))
      (ag', site', state')
        ...
        | list ->
          begin
            let error, (handler, _, _c) =
              declare_site_with_internal_states
                parameters
                (error, handler)
                ag_id
                site_name
...
let scan_observables _scan_mixt _parameters remanent _variable = (*TODO*)
  remanent
